...
describe ReviewsController do
  ...
  describe 'POST create' do
    ...
    it 'sets @review with creator and video' do
      post :create, video_id: video.token, review: Fabricate.attributes_for(:review)
    ...
    context 'with valid input' do
      before { post :create, video_id: video.token, review: Fabricate.attributes_for(:review) }
    ...
    context 'invalid input' do
      ...
      it 'renders video show template if invalid input' do
        post :create, video_id: video.token, review: Fabricate.attributes_for(:review, rating: 10)
        expect(response).to render_template('videos/show')
      end
    end

    it_behaves_like 'requires sign in' do
      let(:action) { post :create, video_id: video.token, review: Fabricate.attributes_for(:review) }
    end
  end
end
