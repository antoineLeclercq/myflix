...
describe UserSignup do
  describe '#sign_up' do
    ...
    it 'tries to create the user' do
      customer = double(:customer, successful?: true, customer_token: 'abcdefg123')
    ...
    context 'valid personal info and valid card' do
      let(:customer) { double(:customer, successful?: true, customer_token: 'abcdefg123') }
      before { expect(StripeWrapper::Customer).to receive(:create) { customer } }

      it 'stores the customer token generated by stripe' do
        UserSignup.new(bob).sign_up(stripe_token, invitation_token)
        expect(bob.stripe_customer_token).to eq(customer.customer_token)
      end
